...
#include <avr/io.h> //define registros del ATmega328P
#include <util/delay.h> // funciones de delay
#include <avr/wdt.h> // control del Watchdog Timer
#include <avr/interrupt.h> // manejo de interrupciones (ISR)
#include <avr/pgmspace.h> // manejo de datos en memoria flash.
#include <avr/sleep.h> // funciones de ahorro de energía.
#include <stdint.h> // tipos enteros con tamaño fijo (uint8_t, int16_t).
// funciones  para manejo de strings y buffers.
#include <stdio.h>
#include <string.h>
...
// cuántos ciclos del WDT hacen falta para llegar al intervalo de medida
#define TARGET_CYCLES ((MEASURE_INTERVAL_S + WDT_INTERVAL_S - 1) / WDT_INTERVAL_S)

#define BAUD 9600
#define UBRR_VALUE ((F_CPU/16/BAUD)-1)

#define BME280_ADDR 0x76 // Dirección I2C del sensor (puede ser 0x77)
...
int32_t t_fine; // variable para cálculos de temperatura, presión y humedad.

// Watchdog
volatile uint16_t wdt_cycles = 0; // cuenta cuántos ciclos de WDT pasaron.
volatile uint8_t wdt_woke = 0; // indica que la interrupción del WDT despertó al MCU
...
void sigfox_send_hex_payload(const uint8_t *data, uint8_t len){
	// Convierte los bytes en un string hexadecimal
	char cmd[3*13]; 
	char *p = cmd;
	strcpy(p, "AT$SF=");
	p += 6;
...
//Apaga ADC para ahorrar energía
...
void BME280_write(uint8_t reg, uint8_t data){
	twi_start(); // inicia comunicación I2C
	twi_write_cmd((BME280_ADDR<<1)|0); // envía dirección del BME280 con bit de escritura (0)
	twi_write_dwr(reg); // indica el registro destino
	twi_write_dwr(data); // escribe el dato
	twi_stop();
}
...
// interrupción del wdt
ISR(WDT_vect){
	wdt_cycles++; // cuenta cuántos ciclos pasaron
	wdt_woke = 1; // indica al loop principal que el MCU fue despertado
}

void wdt_init_interrupt_8s(void){
	cli(); // deshabilita interrupciones globales
	MCUSR &= ~(1<<WDRF); // limpia flag de reset por WDT
	WDTCSR |= (1<<WDCE)|(1<<WDE); // habilita cambio de configuración
	WDTCSR = (1<<WDIE)|(1<<WDP3)|(1<<WDP0); // ~8s, interrupción solo
	sei(); // habilita interrupciones globales
}

// Sleep (power down)
void enter_sleep(void){
	set_sleep_mode(SLEEP_MODE_PWR_DOWN); // mínimo consumo, WDT puede despertar
	sleep_enable(); // Habilita sleep
	sei(); // asegurarse que interrupciones estén habilitadas
	sleep_cpu();´// MCU se para hasta interrupción
	sleep_disable(); // deshabilita sleep cuando despierta
}
...
int main(void){
	...
	while(1){
		wdt_woke = 0;
		enter_sleep(); // detiene CPU hasta que WDT despierte MCU

		// solo sigue si es tiempo de medir y enviar
		if (!wdt_woke) continue;
		if (wdt_cycles < TARGET_CYCLES) continue;

		wdt_cycles = 0;

		// ADC (OPT101) - Habilita ADC y espera estabilización
		adc_enable();
		_delay_ms(2);
		uint16_t luz_adc = adc_read(7); // ADC0 pin
		adc_disable();

		// Medición forzada de BME280
		...
		// Empaqueta datos en 8 bytes para Sigfox
		// format: temp (int16, °C *100), hum (uint16, % *100), pres (uint16, hPa*10), luz (uint16 ADC)
		int16_t t10 = (int16_t)(temp * 100.0f);     
		uint16_t h10 = (uint16_t)(hum * 100.0f);     
		uint16_t p10 = (uint16_t)( (pres/100.0f) * 10.0f ); 
		uint8_t payload[8];
